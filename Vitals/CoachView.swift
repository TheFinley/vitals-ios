...
struct CoachView: View {
    ...
    private var observations: [Observation] {
        ...
        let pr = avg(.protein)
        if pr < t.reference[safe: .protein] * 0.8 { out.append(.init(kind: .eat, title: "More protein at every meal", body: "\(pr.g0) g/day vs. a \(t.reference[safe: .protein].g0) g target. Eggs, Greek yoghurt, chicken, tinned tuna, lentils, tofu — 30–40 g per meal keeps you full and protects muscle while losing weight.")) }
        let fib = avg(.fiber)
        if fib < t.reference[safe: .fiber] * 0.7 { out.append(.init(kind: .eat, title: "Fibre is low", body: "\(fib.g1) g/day; aim for \(t.reference[safe: .fiber].g0) g. Oats, beans, lentils, whole fruit, vegetables and whole-grain bread — fibre also helps lower LDL cholesterol.")) }
        let sf = avg(.satFat)
        if sf > t.reference[safe: .satFat] { out.append(.init(kind: .avoid, title: "Saturated fat over 10% of calories", body: "\(sf.g1) g/day vs. a \(t.reference[safe: .satFat].g0) g limit. Swap butter, fatty cuts, pastries and cream-heavy dishes for olive oil, fish and leaner meat.")) }
        let su = avg(.sugar)
        if su > t.reference[safe: .sugar] { out.append(.init(kind: .avoid, title: "Sugar above the limit", body: "\(su.g0) g/day vs. \(t.reference[safe: .sugar].g0) g. Sweetened drinks, juices, pastries and flavoured yoghurts add up fast — whole fruit instead.")) }
        let na = avg(.sodium)
        if na > t.reference[safe: .sodium] { out.append(.init(kind: .avoid, title: "Sodium is high", body: "\(na.g0) mg/day vs. a 2,300 mg limit. Cured meats, salted fish, instant noodles, soy sauce and packaged snacks are the usual sources.")) }
        let vd = avg(.vitaminD)
        if vd < t.reference[safe: .vitaminD] * 0.5 { out.append(.init(kind: .eat, title: "Vitamin D from food is low", body: "Oily fish (sardines, mackerel, salmon), eggs and fortified milk help; sunlight and a supplement do most of the work — worth asking your doctor.")) }
    ...
    private var swapsCard: some View {
        ...
        return Card(title: "Everyday swaps", subtitle: "Simple defaults", symbol: "arrow.left.arrow.right") {
            list("LEAN ON THESE", Theme.good, [
                ("Grilled fish", "sardines, salmon, mackerel: protein, omega-3 and vitamin D in one plate."),
                ("Eggs, Greek yoghurt, cottage cheese", "cheap high-protein breakfasts that keep you full until lunch."),
                ("Lentils, chickpeas, black beans", "fibre and protein; a bowl of lentil soup or bean chilli is filling for few calories."),
                ("Vegetables first", "fill half the plate before starches; whole grains over refined when you can."),
            ...
            list("GO EASY ON THESE", Theme.bad, [
                ...
                ("Pastries, croissants, biscuits", "saturated fat plus sugar; keep to one or two a week."),
                ("Fried food and fatty cuts", "sausage, bacon, deep-fried snacks push saturated fat and sodium past the limit quickly."),
                ("Creamy or coconut-heavy curries and sauces", "delicious but calorie-dense; halve the cream or the portion."),
